...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    if (role === "admin") {
      // only allow admin if correct secret is provided
      if (adminSecret !== process.env.ADMIN_SECRET) {
        return res.status(403).json({ error: "Invalid admin secret. You cannot register as admin." });
      }
      finalRole = "admin";
    }
    ...
    await db.query(
      ...
      [id, name, email, hashed, finalRole]
    );

    res.status(201).json({ id, name, email, role: finalRole });
